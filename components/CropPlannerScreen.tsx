                                ...
                                {displayPlan.stages.map((stage, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="flex flex-col items-center gap-3 cursor-pointer group" onClick={() => {/* Optional: jump to stage view */}}>
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center border-4 transition-all duration-300 ${isActive ? 'bg-white border-green-600 text-green-600 scale-125 shadow-lg shadow-green-500/20' : isDone ? 'bg-green-500 border-green-500 text-white' : 'bg-slate-50 border-slate-300 text-slate-400'}`}>
                                                {isDone ? '✓' : idx + 1}
                                            </div>
